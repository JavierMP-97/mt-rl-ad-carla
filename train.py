...
parser.add_argument(
    "--log-interval", help="Override log interval (default: -1, no change)", default=-1, type=int
)
...
parser.add_argument(
    "--teleop", action="store_true", default=False, help="Use teleoperation for training"
)
...
if args.trained_agent != "":
    assert args.trained_agent.endswith(".pkl") and os.path.isfile(
        args.trained_agent
...
if not args.teleop:
    ...
    env = make_env(
        args.seed, vae=vae, teleop=args.teleop, n_stack=hyperparams.get("frame_stack", 1)
    )()
...
if args.trained_agent.endswith(".pkl") and os.path.isfile(args.trained_agent):
    ...
    model = ALGOS[args.algo].load(
        args.trained_agent, env=env, tensorboard_log=tensorboard_log, verbose=1, **hyperparams
    )
