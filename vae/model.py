...
class ConvVAE(object):
    ...
    def _build_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.input_tensor = tf.placeholder(tf.float32, shape=[None, 128, 256, 3])
            ...
            print(h.get_shape())
            h = tf.layers.conv2d(
                h, 64, 5, strides=2, padding="SAME", activation=tf.nn.relu, name="enc_conv2"
            )
            print(h.get_shape())
            h = tf.layers.conv2d(
                h, 128, 5, strides=2, padding="SAME", activation=tf.nn.relu, name="enc_conv3"
            )
            print(h.get_shape())
            h = tf.layers.conv2d(
                h, 256, 5, strides=2, padding="SAME", activation=tf.nn.relu, name="enc_conv4"
            )
            print(h.get_shape())
            ...
            if self.is_training == 1 and self.beta != 0:
                self.z = self.mu + self.sigma * self.epsilon
            else:
                self.z = self.mu
                print("BIEN")
            ...
            # Decoder
            h = tf.layers.dense(self.z, 8 * 16 * 256, name="dec_fc")
            h = tf.reshape(h, [-1, 8, 16, 256])
            h = tf.layers.conv2d_transpose(
                h, 128, 5, strides=2, padding="SAME", activation=tf.nn.relu, name="dec_deconv1"
            )
            print(h.get_shape())
            h = tf.layers.conv2d_transpose(
                h, 64, 5, strides=2, padding="SAME", activation=tf.nn.relu, name="dec_deconv2"
            )
            print(h.get_shape())
            h = tf.layers.conv2d_transpose(
                h, 32, 5, strides=2, padding="SAME", activation=tf.nn.relu, name="dec_deconv3"
            )
            print(h.get_shape())
            self.output_tensor = tf.layers.conv2d_transpose(
                h,
                3,
                5,
                strides=2,
                padding="SAME",
                activation=tf.nn.softmax,  # sigmoid,
                name="dec_deconv4",
            )
            print(self.output_tensor.get_shape())

            # train ops
            if self.is_training > 0:
                self.objective_tensor = tf.placeholder(tf.float32, shape=[None, 128, 256, 3])

                self.global_step = tf.Variable(0, name="global_step", trainable=False)

                # Divide loss
                self.out_b, self.out_g, self.out_r = tf.split(
                    self.output_tensor, num_or_size_splits=3, axis=3
                )
                self.obj_b, self.obj_g, self.obj_r = tf.split(
                    self.objective_tensor, num_or_size_splits=3, axis=3
                )

                self.r_loss_b = tf.reduce_mean(
                    tf.reduce_sum(tf.square(self.obj_b - self.out_b), reduction_indices=[1, 2, 3])
                )
                self.r_loss_g = tf.reduce_mean(
                    tf.reduce_sum(tf.square(self.obj_g - self.out_g), reduction_indices=[1, 2, 3])
                )
                self.r_loss_r = tf.reduce_mean(
                    tf.reduce_sum(tf.square(self.obj_r - self.out_r), reduction_indices=[1, 2, 3])
                )

                # reconstruction loss
                """
                self.r_loss = tf.reduce_sum(
                    tf.square(self.objective_tensor - self.output_tensor),
                    reduction_indices=[1, 2, 3]
                )
                """
                self.r_loss = self.r_loss_b + self.r_loss_g * self.green + self.r_loss_r
    ...
    def decode_lines(self, z):
        """
        :param z: (np.ndarray)
        :return: (np.ndarray)
        """
        return self.sess.run(self.out_g, feed_dict={self.z: z})
    ...
    def set_model_params(self, params):
        with self.graph.as_default():
            ...
            for var in t_vars:
                ...
                assign_op = var.assign(p.astype(np.float) / 10000.0)
    ...
    @classmethod
    def load(cls, load_path, is_training=0, **kwargs):
