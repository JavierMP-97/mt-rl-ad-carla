...
class VAEController:
    ...
    def __init__(
        ...
        if z_size is not None:
            ...
            self.target_vae = ConvVAE(
                z_size=self.z_size, batch_size=1, is_training=0, green=green, reuse=False
            )
    ...
    def decode_g(self, arr):
        # assert arr.shape == (self.batch_size, self.z_size), "{} != {}".format(arr.shape, (self.batch_size, self.z_size))
        # Decode
        arr = self.target_vae.decode_lines(arr)
        # Denormalize
        arr = denormalize(arr, mode=self.normalization_mode)
        return arr

    def encode_train(self, observation):
        assert observation.shape == self.input_dimension, "{} != {}".format(
            observation.shape, self.input_dimension
        )
        # Normalize
    ...
    def decode_train(self, arr):
        # assert arr.shape == (self.batch_size, self.z_size), "{} != {}".format(arr.shape, (self.batch_size, self.z_size))
        # Decode
        arr = self.vae.decode(arr)
        # Denormalize
        arr = denormalize(arr, mode=self.normalization_mode)
        return arr

    def decode_train_g(self, arr):
        # assert arr.shape == (self.batch_size, self.z_size), "{} != {}".format(arr.shape, (self.batch_size, self.z_size))
        # Decode
        arr = self.vae.decode_lines(arr)
        # Denormalize
        arr = denormalize(arr, mode=self.normalization_mode)
        return arr
    ...
    def load(self, path, is_training=0):
        self.target_vae = ConvVAE.load(path, is_training=is_training)
