...
class TeleopEnv(object):
    ...
    def __init__(self, env, model=None, is_recording=False, is_training=False, deterministic=True):
        ...
        while not end:
            ...
            if keys[K_o]:
                if (
                    ...
                    print("Optimizing")
                    self.model.optimize(
                        len(self.model.replay_buffer), None, self.model.learning_rate(1)
                    )
            ...
            control_throttle, control_steering = control(
                x, theta, control_throttle, control_steering
            )
            # Send Orders
            ...
            if not (self.is_training and not self.is_manual):
                if self.is_manual and not self.fill_buffer:
                    ...
                    self.current_obs, _, _, _ = donkey_env.postprocessing_step(
                        self.action, self.current_obs, reward, done, info
                    )
                    ...
                    if self.fill_buffer and hasattr(self.model, "replay_buffer"):
                        ...
                        if old_obs.shape[1] == self.current_obs.shape[1]:
                            ...
                            self.model.replay_buffer.add(
                                old_obs, self.action, reward, self.current_obs, float(done)
                            )
    ...
    def update_screen(self, action):
        ...
        self.write_text(
            "Throttle: {:.2f}, Steering: {:.2f}".format(throttle, steering), 20, 0, FONT, WHITE
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--record-folder",
        help="Record folder, where images are saved",
        type=str,
