...
from stable_baselines.common.vec_env import DummyVecEnv, VecFrameStack, VecNormalize
from stable_baselines.ddpg.policies import FeedForwardPolicy as DDPGPolicy
from stable_baselines.sac.policies import FeedForwardPolicy as SACPolicy

from algos import DDPG, PPO2, SAC
from carla_gym.envs.vae_env import DonkeyVAEEnv
from config import (
    FRAME_SKIP,
    LEVEL,
    MAX_CTE_ERROR,
...
class LargeSACPolicy(SACPolicy):
    def __init__(self, *args, **kwargs):
        super(LargeSACPolicy, self).__init__(
...
class TinySACPolicy(SACPolicy):
    def __init__(self, *args, **kwargs):
        super(TinySACPolicy, self).__init__(
            *args, **kwargs, layers=[32, 16], feature_extraction="mlp"
        )
...
class CnnSACPolicy(SACPolicy):
    def __init__(self, *args, **kwargs):
        super(CnnSACPolicy, self).__init__(
            *args, **kwargs, layers=[32, 16], act_fun=tf.nn.elu, feature_extraction="cnn"
        )


class CnnAltSACPolicy(SACPolicy):
    def __init__(self, *args, **kwargs):
        super(CnnAltSACPolicy, self).__init__(
            *args, **kwargs, layers=[50], act_fun=tf.nn.elu, feature_extraction="cnn"
        )
...
class CustomDDPGPolicy(DDPGPolicy):
    def __init__(self, *args, **kwargs):
        super(CustomDDPGPolicy, self).__init__(
            *args, **kwargs, layers=[32, 8], feature_extraction="mlp", layer_norm=True
        )
...
def make_env(seed=0, log_dir=None, vae=None, frame_skip=None, teleop=False, n_stack=1):
    """
    ...
    def sac_callback(_locals, _globals):
        ...
        if _locals["done"] == True:
            ...
            if mean_reward > best_mean_reward:
                if verbose >= 1:
                    print("Saving best mean model")
                _locals["self"].save(save_path + "_mean")
            ...
            if _locals["ep_len"] >= 1000 and episode_rewards[-1] > 2000:
                if verbose >= 1:
                    print("Saving OK model")
                _locals["self"].save("{}_{:02d}".format(save_path, _locals["episode"]))
            if _locals["success"] == True:
                if verbose >= 1:
                    print("Saving successful model")
                _locals["self"].save("{}_done_{:02d}".format(save_path, _locals["episode"]))
